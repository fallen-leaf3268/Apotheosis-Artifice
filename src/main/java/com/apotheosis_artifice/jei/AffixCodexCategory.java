    ...
    public IDrawable getIcon() {
        return new IDrawable() {
            ...
            @Override public void draw(GuiGraphics gfx, int x, int y) {
                net.minecraft.world.item.ItemStack icon = new net.minecraft.world.item.ItemStack(com.apotheosis_artifice.ApotheosisArtificeMod.APOTHEOSIS_REFORGING_TABLE_ITEM.get());
        ...
        for (int i = 0; i < cats.size(); i++) {
            ...
            builder.addSlot(RecipeIngredientRole.OUTPUT, sx, sy)
                ...
                .addTooltipCallback((slotView, tooltip) -> {
                    String key = cat.getName().startsWith("curios:")
                        ? "curios.identifier." + cat.getName().substring(7)
