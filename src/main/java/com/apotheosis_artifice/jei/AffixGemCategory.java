    ...
    public IDrawable getIcon() {
        return new IDrawable() {
            ...
            @Override public void draw(GuiGraphics gfx, int x, int y) {
                ...
                tag.putString("gem", "apotheosis:the_end/endersurge");
                icon.setTag(tag);
        ...
        for (int j = 0; j < rarities.size(); j++) {
            ...
            builder.addSlot(RecipeIngredientRole.INPUT, sx, SLOT_Y)
                ...
                .addTooltipCallback((slotView, tooltip) -> {
                    ...
                    String catLocal;
                    if (entry.category().getName().startsWith("curios:")) {
                        catLocal = Component.translatable("curios.identifier." + entry.category().getName().substring(7)).getString();
                    } else {
